...
from essence_pipeline_utils import (
    conjure_translate_parameter,
    savilerow_translate,
    run_minion,
    parse_minion_solution,
    savilerow_parse_solution,
    conjure_translate_solution,
)
...
def solve_generator(configurationId, paramDict, setting, seed, detailedOutputDir):
    ...
    minionFile = (
        baseFileName + ".minion"
    )  # minion input file, including a negative table saving previously generated solutions of the same generator instance
    minionSolFile = (
        baseFileName + ".solution"
    )  # solution file generated by minion, will be removed afterwards
    auxFile = (
        baseFileName + ".aux"
    )  # aux file generated by SR, will be kept so we don't have to re-generate it next time solving the same generator instance
    eprimeSolFile = (
        baseFileName + ".solution.eprime-param"
    )  # eprime solution file created by SR, will be removed afterwards
    essenceSolFile = (
        baseFileName + ".solution.param"
    )  # essence solution file created by conjure, will be returned as a problem instance
    ...
    if (not os.path.exists(minionFile)) or (os.stat(minionFile).st_size == 0):
        ...
        conjure_translate_parameter(
            eprimeModelFile, paramFile, eprimeParamFile
        )  # translate generator instance from Essence to Essence Prime
        genStatus, genSRTime = savilerow_translate(
            auxFile,
            eprimeModelFile,
            eprimeParamFile,
            minionFile,
            setting["genSRTimeLimit"],
            setting["genSRFlags"],
        )  # translate generator instance from Essence Prime to minion input format
    ...
    # start solving it
    if genStatus == "SRok":
        genStatus, genSolverTime = run_minion(
            minionFile,
            minionSolFile,
            seed,
        ...
        if genStatus == "sat":
            ...
            savilerow_parse_solution(
                eprimeModelFile, minionSolFile, auxFile, eprimeSolFile
            )  # parse solution from minion to Essence Prime
            conjure_translate_solution(
                eprimeModelFile, paramFile, eprimeSolFile, essenceSolFile
            )  # parse solution from Essence Prime to Essence
        delete_file(
            [minionSolFile, eprimeSolFile]
        )  # delete minionSolFile after used, otherwise the negativetable will have duplicated items. eprimeSolFile is removed to make sure that in the next runs, if no solution is found by minion, no Essence solution file is created
    else:
        genSolverTime = 0
    ...
    print("\n")
    log(
        "\nGenerator results: genInstance="
        + os.path.basename(paramFile).replace(".param", "")
        + ", "
        + ", ".join(
            [
                name + "=" + str(localVars[name])
                for name in ["genStatus", "genSRTime", "genSolverTime"]
            ]
        )
    )

    results = {
        "instance": os.path.basename(paramFile),
        "status": genStatus,
        "seed": seed,
        "SRTime": genSRTime,
        "solverTime": genSolverTime,
    }
