...
from minizinc_utils import minizinc_solve, calculate_minizinc_borda_scores, get_minizinc_problem_type, has_better_objective, run_comparator
...
def evaluate_essence_instance_graded(instFile, seed, setting):
    ...
    def get_results():
        ...
        rs = {'insttance': instFile, 'status': status, 'score': score, 'results': results}
        return rs
...
def evaluate_mzn_instance_discriminating(modelFile:str, instFile:str,
                                        scoringMethod:str='complete',
                                        unwantedTypes:list=[],
                                        nEvaluations:int=1,
                                        baseSolver:str='ortools', baseSolverFlags:str="-f", baseMinTime:int=0,
                                        favouredSolver:str='yuck', favouredSolverFlags:str="-f",
                                        totalTimeLimit:int=1200, initSeed:int=None, totalMemLimit=8192):
    """
    ...
    assert scoringMethod in ['complete','incomplete'], "ERROR: scoringMethod " + scoringMethod + " is not supported"
    if len(unwantedTypes)>0:
        for s in unwantedTypes:
            assert s in ['sat','unsat'], "ERROR: elements of unwantedTypes must be in {'sat','unsat'}"
    assert nEvaluations>0

    # initialise info and results
    info = {'base':       {"name": baseSolver,"flags": baseSolverFlags},
    ...
    def get_results():
        assert (score is not None) and (status is not None), "ERROR: score/status is missing"
        rs = {'instance': instFile, 'status': status, 'score':score, 'results': results}
        #print("\n",rs)  
        return rs
    ...
    for solverType in ['favoured','base']:
        ...
        for i in range(nEvaluations):
            ...
            if i>0:
                ...
                if (info[solverType]['name'] in deterministicSolvers) or (flattenStatus!='ok'):
            ...
            print("\n")
            runStatus, runTotalTime, extra = minizinc_solve(modelFile, instFile, info[solverType]['name'], info[solverType]['flags'], seed, totalTimeLimit, totalMemLimit)
                ...
                # otherwise, check if two solvers or two runs of the same solvers return different answers
                    ...
                    if instanceType!=extra['instanceType']:
                        if correctedType is None:
                            ...
                            c_runStatus, c_runTotalTime, c_extra = minizinc_solve(modelFile, instFile, 'chuffed', "-f", None, totalTimeLimit, totalMemLimit)
                            # TODO: what if chuffed fails to solve the instance?
                            assert c_extra['instanceType'] in ['sat','unsat'], "ERROR: inconsistent results between solvers or between runs of the same solvers, and the third solver (chuffed) fails to determine which one is correct"
                        ...
                        if instanceType == correctedType:
                            ...
                            print(f"WARNING: incorrect results by {solver} on {instFile} with seed {seed}. Results returned: {extra['instanceType']}, while chuffed returns {correctedType}")
                            runStatus = "ERR"
                            ...
                            for st in results.keys():
                                for r in results[st]['runs']:
                                    if r['extra']['instanceType'] == instanceType:
                                        print(f"WARNING: incorrect results by {info[st]['name']} on {instFile} with seed {r['seed']}. Results returned: {r['extra']['instanceType']}, while chuffed returns {correctedType}")
                                        r['status'] = "ERR"
                        # assign the correct type 
                        instanceType = correctedType

            results[solverType]['runs'].append({"seed":seed,"status":runStatus,"time":runTotalTime,"extra":extra})
            ...
            if optimal:
                for r in toCheckResults:
                    if r['status'] in ['S','C']:
                        ...
                        if has_better_objective(r['extra']['objs'][-1][1], optimal, problemType):
                            print(f"WARNING: incorrect objective value by {toCheckSolver} on {instFile}. Best objective returned: {r['extra']['objs'][-1][1]}, while ortools returns {optimal}")
                            r['status'] = 'ERR'
    ...
    for i in range(nEvaluations):
        ...
        bordaScores = calculate_minizinc_borda_scores(baseResults['status'], favouredResults['status'],
                                        baseResults['time'], favouredResults['time'],
                                        problemType,
                                        baseResults['extra']['objs'],favouredResults['extra']['objs'],
                                        True)
        print(bordaScores)
    ...
    if favouredSum == 0:
        ...
        assert favouredSum == nEvaluations # the best type of instance we can achieve, where the base solver fails and the favoured solver succeeds
        score = -99999
    else:
        score = -favouredSum/baseSum # when both solvers succeeds at solving the instance, we maximise the ratio of their scores

    status = 'ok'
    return score, get_results()


def evaluate_mzn_instance_graded(modelFile:str, instFile:str,
                                    unwantedTypes:list=[],
                                    nEvaluations:int=1,
                                    solver:str='ortools', solverFlags:str="-f", solverType:str="complete",
                                    minTime:int=10, timeLimit:int=1200,
                                    initSeed:int=None,
                                    oracleSolver:str=None, oracleSolverFlags:str="-f", oracleSolverTimeLimit:int=3600, memLimit=8192):
    """
    ...
    if len(unwantedTypes)>0:
        for s in unwantedTypes:
            assert s in ['sat','unsat'], "ERROR: elements of unwantedTypes must be in {'sat','unsat'}"
    assert nEvaluations>0
    assert solverType in ['complete','incomplete'], "ERROR: solver type must be either complete or incomplete"
    if solverType == 'incomplete':
        assert oracleSolver!=None, "ERROR: for incomplete solver, an oracle solver must be used"
    assert minTime < timeLimit, "ERROR: min solving time must be less than total time limit"
    ...
    if (solverType=='incomplete') and ('unsat' in unwantedTypes):
        smallTimelimit = 120
        oracleRunStatus, oracleRunTotalTime, oracleExtra = minizinc_solve(modelFile, instFile, oracleSolver, oracleSolverFlags, seed, smallTimelimit, memLimit)
    ...
    for i in range(nEvaluations):
        ...
        print("\n")
        runStatus, runTotalTime, extra = minizinc_solve(modelFile, instFile, solver, solverFlags, seed, timeLimit, memLimit)
        results['main']['runs'].append({"seed":seed,"status":runStatus,"time":runTotalTime,"extra":extra})
        ...
        if runStatus in ['S','C']:
            if instanceType is None:
                instanceType = extra['instanceType']
            elif instanceType != extra['instanceType']: # inconsistent results between runs, return immediately
    ...
    # get the median run
    results['main']['runs'] = sorted(results['main']['runs'], key=cmp_to_key(run_comparator))
    ...
    if oracleSolver:
        ...
        print("\nRunning the oracle")
        oracleRunStatus, oracleRunTotalTime, oracleExtra = minizinc_solve(modelFile, instFile, oracleSolver, oracleSolverFlags, seed, oracleSolverTimeLimit, memLimit)
        results['oracle']['runs'].append({"status":oracleRunStatus, "time": oracleRunTotalTime, 'extra': oracleExtra})
        ...
        for r in results['main']['runs']:
            # instance type
            #print(r)
            if (r['status'] in ['S','C']) and (r['extra']['instanceType'] != oracleExtra['instanceType']):
...
def main():
    ...
    genStatus, genSolFile, genMinionFile, genMinionSolString, genResults = solve_generator(configurationId, paramDict, setting['generatorSettings'], seed, detailedOutputDir)
    ...
    assert experimentType in ['graded','discriminating'], "ERROR: invalid experimentType " + experimentType

    modelType = os.path.basename(setting['generalSettings']['modelFile']).split('.')[-1]
    assert modelType in ['essence','mzn'], "ERROR: " + modelFile + ": model type not recognised (must be either .essence or .mzn)"

    def get_unwanted_types():
        gradedTypes = setting['evaluationSettings']['gradedTypes'].strip().replace(' ','').replace('\t','')
        assert gradedTypes in ['both','sat','unsat','sat,unsat','unsat,sat']
        if gradedTypes in ['both','sat,unsat','unsat,sat']:
            return []
        if gradedTypes=='sat':
            return ['unsat']
        return ['sat']
    ...
    if modelType=='essence':
        ...
        score, instanceResults = globals()[evaluationFunctionName](instFile, seed, setting['evaluationSettings'])
    else:
        ...
        if experimentType=='discriminating':
            assert es['baseSolver']['totalTimeLimit']==es['favouredSolver']['totalTimeLimit'], "ERROR: for mininzinc experiments, please set the same time limit for both base and favoured solvers"
            score, instanceResults = evaluate_mzn_instance_discriminating(modelFile='problem.mzn', instFile=mznInstFile,
                                                                            scoringMethod=es['scoringMethod'],
                                                                            unwantedTypes=get_unwanted_types(),
                                                                            nEvaluations=es['nEvaluations'],
                                                                            baseSolver=es['baseSolver']['name'], baseSolverFlags=es['baseSolver']['solverFlags'],baseMinTime=es['baseSolver']['solverMinTime'],
                                                                            favouredSolver=es['favouredSolver']['name'], favouredSolverFlags=es['favouredSolver']['solverFlags'],
            ...
            score, instanceResults = evaluate_mzn_instance_graded(modelFile='problem.mzn', instFile=mznInstFile,
                                                                    unwantedTypes=get_unwanted_types(),
                                                                    nEvaluations=es['nEvaluations'],
                                                                    solver=es['solver'],
                                                                    solverFlags=es['solverFlags'], solverType=es['solverType'],
                                                                    minTime=es['minTime'], timeLimit=es['totalTimeLimit'],
                                                                    initSeed=seed,
                                                                    oracleSolver=oracleSolver, oracleSolverFlags=oracleSolverFlags, oracleSolverTimeLimit=oracleSolverTimeLimit)
