...
: <<'COMMENT'
 ...
 Runs all scripts put in ./push_graded_essence_tests and makes sure that the run contains provided lines.

 Each script involves a full run of the vessel_loading problem, and each one uses a different solver. 

 Solvers tested with include chuffed, cpsat, gecode, picat, and yuck. 

 This script runs less intensive tests (vessel_loading runs with the small generator), and is intended for pushes to any branch.
COMMENT
...
for file in push_graded_essence_tests/*; do
    ((testsRun++))
    # Check if file
    if [[ -f "$file" ]]; then
