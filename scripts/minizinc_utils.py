...
def check_solution_consensus(
    modelFile: str, instFile: str, lastSol: str, solvers: list, verbose: bool = False
):
    """
    ...
    * solvers: list of solvers to check the solution with, represented as strings to pass to minizinc
    ...
    if verbose:
        print(f"creating and solving temporary model in {tf.name}")
    for line in lines:
    ...
    for solver in solvers:
        ...
        if status not in ["S", "C"]:
            if verbose:
                print(
                    f"checked with solver: {solver} cannot seem to find a solution for whatever reason."
                )
        else:
            checked = True
            ...
            if extra["instanceType"] == "sat":
                valid = True
                if verbose:
                    print(f"checked with solver: {solver} agreed.")
            else:
                if verbose:
                    print(f"checked with solver: {solver} disagreed")
    assert (
        checked
    ), "ERROR: none of the checking solvers ({solvers_to_check}) can confirm results of this run ({modelFile}, {instFile})."
    os.remove(tf.name)
    return valid
...
def minizinc_solve(
    ...
    if use_runsolver:
        cmd = (
            f"runsolver -w {runsolver_tmp_file} -d {runsolver_delay} --wall-clock-limit {timeLimit} --vsize-limit {memLimit} "
            + cmd
        )
    ...
    with subprocess.Popen(
        ...
        for line in p.stdout:
            ...
            if "_objective" in line:
                ...
                not line.startswith("%") and (" = " in line) and (line.endswith(";"))
            ):  # recover solution
                lhs, rhs = line.split(" = ")
                lastSol[lhs] = rhs
            sys.stdout.flush()
    ...
    if use_runsolver:
        with open(runsolver_tmp_file) as f:
            for index, line in enumerate(f):
                ...
                if (
                    "Maximum wall clock time exceeded" in line
                    or "Maximum VSize exceeded" in line
                ):
    ...
    if (solvers_to_check is not None) and (len(solvers_to_check) > 0):
        if extra["instanceType"] == "sat":
            consensus = check_solution_consensus(
                modelFile, instFile, lastSol, solvers_to_check, verbose=verbose
            )
            if consensus is False:
                if verbose:
                    print("There is no consensus on the solution correctness")
                status = "ERR"
            ...
            if verbose:
                print(
                    "There is no solution because its unsat, so won't solve that ... too expensive."
                )
...
def calculate_minizinc_borda_scores(
    status1: str,
    status2: str,
    time1: float,
    time2: float,
    ...
    if problemType == "SAT":
        ...
        # instance is solved/unsolvable by both solvers
            ...
            if solved(status1) and solved(status2):
                ...
                scores["complete"] = scores["incomplete"] = (
                    time2 / (time1 + time2),
                    time1 / (time1 + time2),
                )
    ...
    # calculate scores for optimisation problems
        ...
        if better_optimisation_complete(status1, status2, objs1, objs2):
            ...
            # both solvers give equal solution quality but without optimality proof
                ...
                lastTime2 = objs2[-1][0]
                scores["complete"] = (
                    lastTime2 / (lastTime1 + lastTime2),
                    lastTime1 / (lastTime1 + lastTime2),
                )
        ...
        if better_optimisation_incomplete(status1, status2, objs1, objs2):
            ...
            # both solvers give equal solution quality
                ...
                assert (
                    objs1[-1][1] == objs2[-1][1]
                )  # check if both solvers give the same solution quality
                lastTime1 = objs1[-1][0]
                lastTime2 = objs2[-1][0]
                scores["incomplete"] = (
                    lastTime2 / (lastTime1 + lastTime2),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help='list of solvers to use in a comma separated style: name1,name2,name3... Pass an empty string ("") to not check the solution',
    )

    # read from command line args
    args = parser.parse_args()

    solvers = args.solverCheck.split(",")
