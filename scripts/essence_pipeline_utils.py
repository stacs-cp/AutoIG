...
solverInfo['boolector'] = {'timelimitUnit': 's',
                            'timelimitPrefix': '--time=',
                            'randomSeedPrefix': '--seed='}
solverInfo['kissat'] = {'timelimitUnit': 's',
                            'timelimitPrefix': '--time=',
                            'randomSeedPrefix': '--seed='}
...
def call_conjure_solve(essenceModelFile, eprimeModelFile, instFile, setting, seed):
    ...
    if ('GC overhead limit exceeded' in cmdOutput) or ('OutOfMemoryError' in cmdOutput) or ('insufficient memory' in cmdOutput):
        ...
        status = 'solverMemOut'
    elif ('Sub-process exited with error code:139' in cmdOutput) and (setting['abortIfSolverCrash'] is False):
        status = 'solverCrash'
    elif returnCode != 0:
        raise Exception(cmdOutput)
    ...
    if os.path.isfile(infoFile):
        ...
        # parse SR info file
        infoStatus, SRTime, solverTime = parse_SR_info_file(infoFile, timelimit=setting['solverTimelimit'])
